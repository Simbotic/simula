...
pub struct MyEditorState(
    pub GraphEditorState<MyNodeData, MyDataType, MyValueType, MyNodeTemplate, MyGraphState>,
);

pub fn egui_update(
    mut egui_context: ResMut<EguiContext>,
    mut states: Query<(&mut MyEditorState, &mut MyGraphState)>,
) {
    for (mut editor_state, mut user_state) in states.iter_mut() {
        ...
        let graph_response = egui::Window::new("Node Graph")
            .show(ctx, |ui| {
