...
use bevy_inspector_egui::{egui::{self}, reflect_inspector};
use egui_node_graph::*;
    ...
    T: BehaviorSpawner,
{
    ...
    fn build_node(
            ...
            MyNodeTemplate::Behavior(behavior) => {
                match behavior.typ() {
                    ...
                    BehaviorType::Composite => {
                        ...
                        graph.add_output_param(node_id, "0".into(), MyDataType::Flow);
                        graph.add_output_param(node_id, "1".into(), MyDataType::Flow);
                        graph.add_output_param(node_id, "2".into(), MyDataType::Flow);
                    }
                }
            }
        }
        ...
        // let output_scalar = |graph: &mut MyGraph<T>, name: &str| {
        //     graph.add_output_param(node_id, name.to_string(), MyDataType::Scalar);
        // };
        // let output_vector = |graph: &mut MyGraph<T>, name: &str| {
    ...
    T: BehaviorSpawner,
{
    ...
    fn all_kinds(&self) -> Vec<Self::Item> {
        ...
        let mut kinds: Vec<MyNodeTemplate<T>> = T::list().into_iter().map(|t| MyNodeTemplate::Behavior(t)).collect();
    ...
    T: BehaviorSpawner,
{
    ...
    ) -> Vec<NodeResponse<MyResponse, MyNodeData<T>>>
    where
        T: BehaviorSpawner,
        ...
        if let Some(node) = graph.nodes.get(node_id) {
            match &node.user_data.template {
                ...
                MyNodeTemplate::Behavior(behavior) => {
                    ...
                    reflect_inspector::ui_for_value(behavior.reflect(), ui, &type_registry);
                    // let button = egui::Button::new(egui::RichText::new("👁 Active").color(egui::Color32::BLACK))
                    //     .fill(egui::Color32::GOLD);
                    // if ui.add(button).clicked() {
                    //     user_state.active_node = Some(node_id);
                    // }
                }
            }
        }
...
type MyGraph<T> = Graph<MyNodeData<T>, MyDataType, MyValueType<T>>;
// type MyEditorState =
...
// pub struct MyGraph(pub Graph<MyNodeData, MyDataType, MyValueType>);
