            ...
            for event in events.iter() {
                match event {
                    ...
                    CameraEvents::Pan(delta) => {
                        ...
                        let up_dir = -Vec3::cross(transform.rotation * Vec3::X, Vec3::Y);
                        let pan_vector = (delta.x * right_dir + delta.y * up_dir)
                            * camera.pan_sensitivity
                            * time.delta_seconds();
                        camera.center += pan_vector;
                        camera.center.x = camera.center.x.clamp(-2048.0, 0.0);
                        camera.center.z = camera.center.z.clamp(0.0, 2048.0);
                    }
                    _ => {}
                }
            }
                ...
                if camera.enabled {
                    if let CameraEvents::Zoom(distance) = event {
                        camera.distance = (camera.distance
                            * camera.zoom_sensitivity.powf(*distance))
                        .clamp(10.0, 85.0);
                    }
                }
            }
